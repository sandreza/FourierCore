...
##
#=
hfile = h5open("generative_response.hdf5", "r")
generative_response = read(hfile["generative response"])
close(hfile)
=#
a_linear_response = zeros(N, N, 100)
a_hack_score_response = zeros(N, N, 100)
a_numerical_response = zeros(N, N, 100)
# a_generative_response = zeros(N, N, 100)
for i in 1:100 
    a_linear_response[:, :, i] .= reshape(lr[i][:, 1], (N, N))
    a_hack_score_response[:, :, i] .= reshape(hsr[i][:, 1], (N, N))
    a_numerical_response[:, :, i] .= nr[:, :, 32*(i-1)+1]
    # a_generative_response[:, :, i] .= generative_response[:, :, i]
end
...
# hfile["generative response"] = a_generative_response
close(hfile)
